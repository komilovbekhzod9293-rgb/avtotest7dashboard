...
export function Oquvchilar() {
  ...
  const upcomingTomorrow = new Date(now); upcomingTomorrow.setDate(now.getDate() + 1);
  const upcomingDayAfter = new Date(now); upcomingDayAfter.setDate(now.getDate() + 2);
  const targetDay = imtihonFilter === "Ertaga" ? upcomingTomorrow : upcomingDayAfter;
  const upcomingImtihon = activeStudents.filter(s => {
    if (!s.imtihon) return false;
    const d = parseSheetDate(s.imtihon);
    return d && sameDay(d, targetDay);
  });
    ...
    <div>
      ...
      {editStudent && (
        <div className="fixed inset-0 bg-foreground/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl border border-border w-full max-w-sm max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm font-medium">Chetlatildi (kontrakt buzildi)</span>
              </label>
