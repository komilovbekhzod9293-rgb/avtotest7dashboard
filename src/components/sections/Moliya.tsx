...
interface PravaOnExpense { id: string; summa: number; filial: string | null; izoh: string | null; created_at: string; }
...
export function Moliya() {
  ...
  const [pravaOnExpenses, setPravaOnExpenses] = useState<PravaOnExpense[]>([]);
  const [expLoading, setExpLoading] = useState(false);
  const [showExpForm, setShowExpForm] = useState(false);
  const [expSumma, setExpSumma] = useState("");
  const [expFilial, setExpFilial] = useState("Novza");
  ...
  const fetchPravaOnExpenses = () => {
    setExpLoading(true);
    fetch(`${SUPABASE_URL}/rest/v1/prava_on_expenses?order=created_at.desc`, {
      headers: { "apikey": SUPABASE_KEY, "Authorization": `Bearer ${SUPABASE_KEY}` },
    })
    .then(function(r) { return r.json(); })
    .then(function(data) { setPravaOnExpenses(Array.isArray(data) ? data : []); })
    .catch(function() {})
    .finally(function() { setExpLoading(false); });
  };

  async function submitPravaOnExpense() {
    const summaNum = parseInt(expSumma.replace(/\s/g, ""));
    if (!summaNum || isNaN(summaNum)) { setExpResult("Summani kiriting"); return; }
    setExpSaving(true); setExpResult(null);
    try {
  ...
  useEffect(function() { fetchData(); fetchRejadagi(); fetchPayments(); fetchPravaOnExpenses(); }, []);
