...
import { useMoliyaData } from "@/hooks/useGoogleSheets";
...
export function Moliya() {
  const { rows, loading, error } = useMoliyaData();
  ...
  rows.forEach((r) => {
    ...
    const key = UZ_MONTHS[monthIdx] ?? r.sana;
